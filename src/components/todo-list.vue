      ...
      <ul class="todo-list">
        ...
        v-bind:class="{completed : todo.complete}">
          <item 
            ...
            v-on:complete="toggleTodoComplete">
          </item>
        </li>
      </ul>
    </section>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const itemCountMessage = computed(() => {
      return todos.value.length == 1 ? 'item' : 'items' 
    })

    function connect() {
      socket = io(props.socketResource || '', {transports: ['websocket']});
    ...
    function addTodo() {
      ...
      if (newTodo.value.length > 0) socket.emit('newItem', newTodo.value);
      newTodo.value = "";
    }
